...
type Rate = { platform: SocialPlatform; pct: number | null; delta: number | null; followers: number | null; up: boolean };
...
export default function GrowthRateCard() {
  ...
  const rates = useMemo<Rate[]>(() => {
    return SOCIAL_PLATFORMS.map((platform) => {
      ...
      if (rows.length < 8) return { platform, pct: null, delta: null, followers: null, up: true };
      const latest = rows[rows.length - 1];
      const weekAgo = rows[rows.length - 8];
      const delta = latest.followers - weekAgo.followers;
      const pct = weekAgo.followers > 0 ? (delta / weekAgo.followers) * 100 : null;
      return { platform, pct, delta, followers: latest.followers, up: (pct ?? 0) >= 0 };
    });
  }, [data.daily]);
  ...
  return (
    <WidgetCard title="Wachstumsrate" badge={isMock ? "Mock-Daten" : "Live"} badgeTone={isMock ? "neutral" : "accent"}>
      <div className="grid grid-cols-2 gap-5">
        {rates.map(({ platform, pct, delta, followers, up }) => (
          <div key={platform} className="min-w-0 py-1 text-center">
            <p className="truncate text-xs uppercase tracking-[0.12em] text-muted">
              {SOCIAL_PLATFORM_LABEL[platform]}
            </p>
            <p
              className={`mt-2 flex items-center justify-center gap-1.5 font-mono text-stat-sm tabular-nums tracking-tight ${
            ...
            <p className="mt-3 font-mono text-sm tabular-nums text-foreground">
              {delta === null ? "—" : `${delta >= 0 ? "+" : "−"}${socialInt.format(Math.abs(delta))}`}
            </p>
            <p className="mt-0.5 text-[11px] text-muted">
              Follower / Woche{followers !== null && ` · ${socialInt.format(followers)} gesamt`}
            </p>
          </div>
        ))}
      </div>
